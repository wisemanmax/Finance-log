...
const TOUR_SLIDES=[
  {icon:"💰",grad:"135deg,#00f5a0,#00d9f5",title:"Welcome to FinanceLog",desc:"Your personal finance companion. Track every dollar, set budgets, and watch your wealth grow — all stored privately on your device."},
  {icon:"💸",grad:"135deg,#a78bfa,#818cf8",title:"Log Transactions Instantly",desc:"Add income and expenses in seconds. Tag by category, account, or note — then see exactly where your money goes."},
  {icon:"📊",grad:"135deg,#f59e0b,#ef4444",title:"Budgets That Actually Work",desc:"Set monthly limits per category. Real-time progress bars show you how much you have left before you overspend."},
  {icon:"📈",grad:"135deg,#00d9f5,#3b82f6",title:"Track Your Net Worth",desc:"Add all your accounts — checking, savings, investments, debts. Watch your net worth climb month over month."},
];

const CURRENCIES=[
  {id:"USD",symbol:"$",name:"US Dollar"},{id:"EUR",symbol:"€",name:"Euro"},{id:"GBP",symbol:"£",name:"British Pound"},
  {id:"CAD",symbol:"C$",name:"Canadian Dollar"},{id:"AUD",symbol:"A$",name:"Australian Dollar"},
  {id:"JPY",symbol:"¥",name:"Japanese Yen"},{id:"INR",symbol:"₹",name:"Indian Rupee"},{id:"BRL",symbol:"R$",name:"Brazilian Real"},
];

export function Onboarding({d}){
  // ─── Onboarding state ───
  const [step,setStep]=useState(0); // 0=welcome, 1=tour, 2=currency, 3=goals, 4=profile, 5=pin, then verify
  ...
  const finish=async()=>{
    ...
    try{
      ...
      const payload={
        ...profile,units:"lbs",primaryGoal:"savings",
  ...
  const verifyConfirmCode=async()=>{
    ...
    try{
      ...
      if(json.verified||json.already_verified){
        setVerifyStep("done");LS.set("fl-email-verified","true");
  ...
  const applyRestore=(cloud,pinUsed)=>{
    const merged={
      transactions:cloud.transactions||[],accounts:cloud.accounts||[],
      budgets:cloud.budgets||{},goals:cloud.goals||[],recurring:cloud.recurring||[],
    ...
    if(merged.currency)d({type:"SET_CURRENCY",currency:merged.currency});
    ...
    // Route to app-specific setup steps instead of skipping to main app
    setReturningUser(true);
    setShowSignIn(false);
    setStep(1); // tour first, then app-specific steps
  };
  ...
  const linkSSO=()=>{
    ...
    setStep(2); // skip to currency after SSO
  };
        ...
        {verifyStep==="done"?(
          ...
          <div style={{maxWidth:360,width:"100%",animation:"fadeUp .4s ease"}}>
            ...
            <div style={{display:"flex",justifyContent:"space-between",marginTop:12}}>
              ...
              <button onClick={()=>{LS.set("fl-email-verified","skipped");d({type:"ONBOARDED"});}} style={{background:"none",border:"none",color:V.text3,fontSize:12,cursor:"pointer",fontFamily:V.font}}>Skip for now</button>
            </div>
          </div>
        )}
      </div>
  ...
  return(
    <div style={{minHeight:"100vh",background:V.bg,display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",padding:24}}>
      <div style={{maxWidth:400,width:"100%"}}>
        ...
        {step===0&&(
          <div style={{textAlign:"center",animation:"fadeUp .5s ease"}}>
            <div style={{width:72,height:72,borderRadius:22,background:`linear-gradient(135deg,${V.accent},${V.accent2})`,
              ...
              {Icons.dollar({size:32,color:V.bg})}
            </div>
            <div style={{fontSize:30,fontWeight:800,color:V.text,letterSpacing:"-.03em"}}>Finance<span style={{color:V.accent}}>Log</span></div>
            <div style={{fontSize:14,color:V.text3,marginBottom:24,lineHeight:1.7}}>Track spending, build budgets,<br/>and grow your wealth.</div>
        ...
        {step===2&&(
          <div style={{animation:"fadeUp .4s ease"}}>
            <div style={{textAlign:"center",marginBottom:20}}>
              ...
              <div style={{fontSize:20,fontWeight:800,color:V.text}}>Choose Your Currency</div>
              <div style={{fontSize:12,color:V.text3}}>This sets the default for all amounts</div>
            </div>
            <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:8,marginBottom:20}}>
              {CURRENCIES.map(c=>(
                <button key={c.id} onClick={()=>setCurrency(c.id)} style={{padding:"14px 12px",borderRadius:14,
                  background:currency===c.id?`${V.accent}12`:"rgba(255,255,255,0.03)",
                  border:`1.5px solid ${currency===c.id?V.accent:V.cardBorder}`,cursor:"pointer",textAlign:"left"}}>
                  <div style={{fontSize:16,fontWeight:700,color:currency===c.id?V.accent:V.text}}>{c.symbol} {c.id}</div>
                  <div style={{fontSize:10,color:V.text3}}>{c.name}</div>
                </button>
              ))}
            </div>
        ...
        {/* ─── Step 3: Budget & Savings Goals ─── */}
        {step===3&&(
          <div style={{animation:"fadeUp .4s ease"}}>
            <div style={{textAlign:"center",marginBottom:20}}>
              <div style={{fontSize:36,marginBottom:8}}>🎯</div>
              <div style={{fontSize:20,fontWeight:800,color:V.text}}>Set Your Goals</div>
              <div style={{fontSize:12,color:V.text3}}>Optional — you can always change these later</div>
            </div>
            <Field label="Monthly Budget (total)" type="number" value={monthlyBudget} onChange={setMonthlyBudget} placeholder="e.g. 3000" unit={CURRENCIES.find(c=>c.id===currency)?.symbol||"$"}/>
            <Field label="Savings Goal" type="number" value={savingsGoal} onChange={setSavingsGoal} placeholder="e.g. 10000" unit={CURRENCIES.find(c=>c.id===currency)?.symbol||"$"}/>
            <Btn full onClick={()=>{
              // Apply app-specific settings
              d({type:"SET_CURRENCY",currency});
              if(monthlyBudget){const perCat=Math.round(parseFloat(monthlyBudget)/5);d({type:"SET_BUD",budgets:{food:perCat,transport:perCat,entertainment:perCat,shopping:perCat,other:perCat}});}
              if(savingsGoal){d({type:"ADD_GOAL",goal:{id:uid(),name:"Savings Goal",target:parseFloat(savingsGoal)||1000,current:0,icon:"💰",created:today()}});}
              if(returningUser){d({type:"ONBOARDED"});}else{setStep(4);}
            }}>{returningUser?"Finish Setup":"Next"}</Btn>
          ...
          <div style={{animation:"fadeUp .4s ease"}}>
            ...
            {!profileValid&&profile.firstName&&(
              <div style={{fontSize:11,color:V.danger,textAlign:"center",marginBottom:8}}>
                {!nameClean?"Please use appropriate names":!emailValid?"Enter a valid email":!ageValid?"Must be 13+ to use FinanceLog":"Fill in all required fields"}
              </div>
            )}
